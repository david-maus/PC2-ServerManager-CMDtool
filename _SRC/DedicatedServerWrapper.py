...
# -*- coding: UTF-8 -*-
"""
Project Cars 2 / Dedicated Server Wrapper CMD Tool & Weather Randomizer.
...
if getattr(sys, 'frozen', False):
    ...
    folderScript = os.path.abspath(os.path.join(folderCurrent,
                                   '../', '../'))
else:
...
if sys.platform == "linux" or sys.platform == "linux2":
    serverExe = os.path.abspath(os.path.join(folderScript,
                                '../', 'DedicatedServerCmd.elf'))
elif sys.platform == "win32":
    serverExe = os.path.abspath(os.path.join(folderScript,
                                '../', 'DedicatedServerCmd.exe'))

rotateFile = os.path.abspath(os.path.join(folderScript, '../',
                             'lua_config', 'sms_rotate_config.json'))
rotateCache = os.path.abspath(os.path.join(folderScript, '../',
                              'lua_config', 'sms_rotate_data.json'))
statsCache = os.path.abspath(os.path.join(folderScript, '../',
                             'lua_config', 'sms_stats_data.json'))
tracksFile = os.path.abspath(os.path.join(folderScript,
                             'help', 'tracks.txt'))
serverFile = os.path.abspath(os.path.join(folderScript,
                             '../', 'server.cfg'))
serverDir = os.path.abspath(os.path.join(folderScript, '../'))

iniPath = os.path.join(folderScript, 'configs', iniFile)

incPath = os.path.join(folderScript, 'inc')
...
def killServerWin():
    ...
    try:
        for line in subprocess.check_output('tasklist /v /fo csv | findstr /i "' + ServerName + '"', shell=True, encoding='utf8').split('\n'):
            if (line == ""):
                pass
            else:
                line = line.split(",")
                processPID = line[1]
                processPID = processPID.replace('"', '')

                processDir = subprocess.check_output('wmic process where "ProcessID=' + processPID + '" get ExecutablePath', shell=True, encoding='utf8')
                processDir = processDir.replace('ExecutablePath', '')
